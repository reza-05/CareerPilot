...
export default function JobCard({ job }: { job: any }) {
  ...
  return (
    <div className="bg-neutral-900 p-6 rounded-2xl border border-neutral-800">
      ...
      <button onClick={() => setShow(!show)} className="mt-4 text-blue-400 text-xs hover:underline">
        {show ? "Hide AI Analysis" : "View AI Analysis"}
      </button>
      
      {show && <p className="mt-4 text-neutral-300 text-sm italic">{job.reasoning}</p>}
      <a href={job.url} target="_blank" className="mt-4 block text-blue-500 font-bold">Apply Now</a>
    </div>
  );
}